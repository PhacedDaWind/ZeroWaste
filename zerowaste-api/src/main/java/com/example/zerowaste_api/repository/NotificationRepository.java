...
public interface NotificationRepository extends JpaRepository<Notification,Long> {
    ...
    @Query("SELECT u FROM Notification u LEFT JOIN u.user e WHERE e.id=:userId AND (:notifType IS NULL OR u.notifType = :notifType) ORDER BY u.createdAt DESC")
    ...
    @Query("SELECT u FROM Notification u LEFT JOIN u.user e WHERE e.id=:userId AND u.markAsRead=false ORDER BY u.createdAt DESC ")
    List<Notification> findByUnread(@Param("userId") Long userId);

    //Delete whole notification for a specific user
    @Transactional
    @Modifying
    @Query("DELETE FROM Notification u WHERE u.user.id=:usersId ")
